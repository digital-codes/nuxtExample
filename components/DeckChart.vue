      ...
      &copy; <a href="https://www.openstreetmap.org/copyright" target="_blank">OpenStreetMap</a>
        -->
            ...
            <span class="attribution">
                <template v-for="(item, idx) in attributionItems" :key="idx">
                    <!-- Plain text pieces -->
                    <span v-if="item.text">{{ item.text }}</span>

                    <!-- Link pieces -->
                    <a v-else :href="item.href" :target="item.target" rel="noopener noreferrer">{{ item.label }}</a>
                </template>
            </span>
...
// Each entry describes one link (or plain text)
const attributionList = [
    {
        // plain text before the first link (optional)
        text: '© ',
    },
    {
        href: 'https://www.bkg.bund.de',
        label: `GeoBasis‑DE/BKG (${new Date().getFullYear()})`,
        target: '_blank',
    },
    {
        // separator – you can also render it as plain text
        text: ' | ',
    },
    {
...
export default defineComponent({
    ...
    setup() {
        ...
        const attributionItems = ref(attributionList)

        // If you ever need a single string (e.g., for an external API),
        // you can compute it from the same source:
        ...
        return {
            cv, deckgl, deck, tiles, tripData,
            startYear, stopYear, startWeek, tm, speed,
            INITIAL_VIEW_STATE, LABEL_VIEW_STATE, mapAttrString, attributionItems, attributionString
        }
    },
})
